...
//! 命令拼装 + ssh config 解析。UI 与 PTY 注入逻辑放在 `app/src/ssh_manager/`
//! 与 `secret_injector` 模块,这里保持纯 Rust、无 warpui 依赖、可单独
//! `cargo test` 跑。
...
pub use ssh_config::{parse_ssh_config, read_ssh_config_file, SshConfigHost};
